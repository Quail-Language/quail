...
public class EventFuncAddHandler extends QBuiltinFunc {


    public EventFuncAddHandler(Runtime runtime, Memory closure) {
        super(
                "addHandler",
                Arrays.asList(
                        new FuncArgument(
    ...
    public EventFuncAddHandler(Runtime runtime) {
    ...
    public QObject action(Runtime runtime, HashMap<String, QObject> args, List<QObject> argList) throws RuntimeStriker {
        ...
        if (!thisManager.getEventHandlers().containsKey(event))
            thisManager.getEventHandlers().put(event, new ArrayList<>());
        thisManager.getEventHandlers().get(event).add(handler);
